...
class DynamicEffBalancedSoftmaxLoss(nn.Module):
    def __init__(self, cls_num_list, tau=0.5):
        super().__init__()

        beta = 0.999
        cls_num_list = torch.tensor(cls_num_list).float()

        cls_prior = 1.0 - torch.pow(beta, cls_num_list)
        cls_prior = cls_prior / cls_prior.sum()

        log_prior = torch.log(cls_prior)

        self.register_buffer("log_prior", log_prior.unsqueeze(0))
        self.tau = tau

    def forward(self, logits, labels,tau):
        logits = logits + tau * self.log_prior
